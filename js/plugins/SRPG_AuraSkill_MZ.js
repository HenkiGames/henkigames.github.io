...
// visualize Aura on movetable
  ...
  const unit = $gameSystem.EventToUnit(userevent.eventId());
  let orix = userevent.posX();
  let oriy = userevent.posY();
  const route = userevent._srpgForceRoute;

  // Partie d'origine : auras des skills et states de l'unité
  ...
  unit[1].skills().forEach(item => {
    if (item.meta.SRPGAuraState && (_showColor === 'true' || item.meta.SRPGShowAura)) {
      $gameTemp.pushAuratoMoveList(item, orix, oriy);
    }
  });

  unit[1].states().forEach(item => {
    if (item.meta.SRPGAuraState && (_showColor === 'true' || item.meta.SRPGShowAura)) {
      $gameTemp.pushAuratoMoveList(item, orix, oriy);
    }
  });

  // 🆕 Partie ajoutée : scan des events sur la map avec balises d’aura visibles
  $gameMap.events().forEach(event => {
    if (!event || !event.page() || !event.list()) return;
    ...
    for (const cmd of list) {
      if (cmd.code === 108 || cmd.code === 408) {
        ...
        if (line.match(/<SRPGAuraState:(\d+)>/i)) {
          dummyMeta.SRPGAuraState = RegExp.$1;
          hasAura = true;
        }
        if (line.match(/<SRPGShowAura:true>/i)) {
          dummyMeta.SRPGShowAura = "true";
        }
        if (line.match(/<SRPGAuraRange:(\d+)>/i)) {
          dummyMeta.SRPGAuraRange = RegExp.$1;
        }
        if (line.match(/<SRPGAuraShape:(\w+)>/i)) {
          dummyMeta.SRPGAuraShape = RegExp.$1;
        }
    ...
    if (hasAura && dummyMeta.SRPGShowAura === "true") {
      const dummyItem = { meta: dummyMeta };
      $gameTemp.pushAuratoMoveList(dummyItem, event.posX(), event.posY());
    }
  });
};
