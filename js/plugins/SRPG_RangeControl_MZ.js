...
//====================================================================
	...
	// Liste des compétences pertinentes pour estimer la portée d’attaque (aperçu ennemi / inspect)
	Game_BattlerBase.prototype.srpgSkillsForRangePreview = function() {
		if (this.isActor()) {
			return this.skills();
		}
		if (this.isEnemy()) {
			...
			addSkill($dataSkills[this.attackSkillId()]);
			this.enemy().actions.forEach(function(a) {
				let sid = a.skillId;
				if (sid === 1) sid = this.attackSkillId();
				addSkill($dataSkills[sid]);
			}, this);
			return list;
		}
		return [];
	...
	// Compétence dont la portée SRPG numérique est la plus grande (hors compétences « toute la carte »)
	Game_BattlerBase.prototype.srpgSkillForMaxRangePreview = function() {
		const skills = this.srpgSkillsForRangePreview();
		let best = $dataSkills[this.attackSkillId()];
		let bestR = best ? this.srpgSkillRange(best) : 0;
		for (let i = 0; i < skills.length; i++) {
			const skill = skills[i];
			if (!skill || this.isAllRangeSkill(skill)) continue;
			const r = this.srpgSkillRange(skill);
			if (r > bestR) {
				bestR = r;
				best = skill;
			}
	...
	// Clic sur ennemi : cases bleues = déplacement, rouges = portée max parmi ses compétences (comme srpgMakeMoveTable mais skill max)
	Game_System.prototype.srpgMakeEnemyInspectMoveTable = function(event) {
		var user = $gameSystem.EventToUnit(event.eventId())[1];
		var item = user.srpgSkillForMaxRangePreview();
		if (!item) item = $dataSkills[user.attackSkillId()];
