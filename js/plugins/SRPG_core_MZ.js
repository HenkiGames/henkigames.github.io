 ...
 * usable:attack,skill,item,equip,wait,swap(split ',')
 * @type string
...
(() => {
    ...
    var _srpgActorCommandList = parameters['srpgActorCommandList'] || 'attack,skill,item,equip,swap,wait';
...
//====================================================================
    ...
    Game_Actor.prototype.noRewards = function() {
        ...
        if (this.currentAction() && this.currentAction().item() && this.currentAction().item().meta.noRewards === 'true') return true;
        return false;
    };
                    ...
                    $gameMap.eventsXy(x, y).forEach(function(event) {
                        if (event.isTriggerIn(triggers) && !event.isErased()) {
                            ...
                            // 選択されたユニットがエネミーの場合
                                ...
                                SoundManager.playOk();
                                $gameTemp.setActiveEvent(event);
                                // Portée de déplacement + portée max (compétences) uniquement — pas de fenêtre statut plein écran
                                if ($gameSystem.srpgMakeEnemyInspectMoveTable) {
                                    $gameSystem.srpgMakeEnemyInspectMoveTable(event);
                                } else {
                                    $gameSystem.srpgMakeMoveTable(event);
                                }
                                $gameSystem.setSubBattlePhase('status_window');
            ...
            if (this._actor) {
                ...
                for (var i = 0; i < list.length; i++) {
                    ...
                    case 'equip':
                        ...
                        break;
                    case 'swap':
                        this.addCommand("Échanger", "exchange");
                        break;
                    case 'wait':
                        this.addWaitCommand();
                        break;
...
//====================================================================
    ...
    Scene_Map.prototype.commandOriginal = function() {
        ...
        const lastActor = $gameActors.actor(actor._actorId);
        var lastSkillId = lastActor._lastUsedSrpgSkillId;
        console.log('last actor => ', lastActor);
        lastActor.action(0).setSkill(lastSkillId);
	...
	Scene_Map.prototype.srpgInvokeMapSkill = function(data) {
		...
		switch (data.phase) {
			...
			case 'animation':
                ...
                if (animationData) {
                    // Animations image par image (frames) : en MZ elles ont souvent displayType 0/1/2
                    // comme l'Effekseer. Pour position === 3 (écran), il faut la même logique « une seule fois »
                    // que la branche MV (voir mapBattleAnimationFlagPos3), sinon chaque cible AoE relance l'anim.
                    const _srpgIsCellAnimation = !!(
                        animationData.frames && animationData.frames.length > 0
                    );
                    const _srpgCellScreen = _srpgIsCellAnimation && animationData.position === 3;
                    // MZ animation (Effekseer)
                    if (animationData.displayType >= 0 && !_srpgCellScreen) {
                    ...
                    // MV (displayType < 0) ou cellulaire plein écran (position 3)
                    } else {
